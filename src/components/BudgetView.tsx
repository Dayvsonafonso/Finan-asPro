      ...
      <Card className="p-6">
        ...
        <Reorder.Group axis="y" values={localCategories} onReorder={handleReorder} className="space-y-6">
          {localCategories.map(cat => {
            ...
            const categoryTransactions = currentMonthTransactions.filter(t => t.type === 'expense' && t.category === cat.name);

            return (
              <Reorder.Item key={cat.id} value={cat} className="p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50 transition-colors relative group">
                <div className="absolute left-1 top-0 bottom-0 w-8 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing text-gray-400">
                    ...
                    {editingId === cat.id ? (
                      <div className="flex items-center space-x-2">
                        <Input 
                          ...
                          autoFocus
                        />
                        <Button size="icon" variant="ghost" onClick={() => handleSave(cat.id)} className="text-green-500 h-9 w-9">
                          <Check className="w-4 h-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => setEditingId(null)} className="text-red-500 h-9 w-9">
                          <X className="w-4 h-4" />
                        </Button>
                      </div>
                    ) : (
                      <div className="flex items-center space-x-3">
                        <div className="text-right">
                          <p className="text-sm font-medium text-gray-900 dark:text-white">Orçado: {formatCurrency(budget)}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            {budget > 0 ? `Restante: ${formatCurrency(budget - spent)}` : 'Sem limite definido'}
                          </p>
                        </div>
                ...
                {budget > 0 && (
                  <div className="mt-2">
                    ...
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                      <div 
                        className={`h-2.5 rounded-full transition-all duration-500 ${isOverBudget ? 'bg-red-500' : 'bg-indigo-600 dark:bg-indigo-400'}`} 
                        style={{ width: `${percent}%` }}
                      ></div>
                    </div>
                  </div>
                      ...
                      <div className="pt-4 border-t border-gray-100 dark:border-gray-800 space-y-2">
                        ...
                        {categoryTransactions.length > 0 ? (
                          categoryTransactions.map(t => (
                            <div key={t.id} className="flex flex-col sm:flex-row sm:items-center justify-between text-sm py-2 px-3 rounded-lg bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 gap-2 sm:gap-0">
